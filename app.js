  ...
  const datasetDirectories = ['0.100_1.0_1.0_1.0_0.0_0_0'];
  ...
  async function loadRealDataset(folderName) {
    ...
    let payload = parseJsonText(new TextDecoder().decode(buffer));
    if (!payload) {
      const isGzip = buffer.byteLength >= 2 && new Uint8Array(buffer)[0] === 0x1f && new Uint8Array(buffer)[1] === 0x8b;
      if (isGzip && typeof DecompressionStream !== 'undefined') {
        const decompressedStream = new Response(
          new Blob([buffer]).stream().pipeThrough(new DecompressionStream('gzip'))
        );
        const decompressedText = await decompressedStream.text();
        payload = parseJsonText(decompressedText);
      }
    }
  ...
  async function loadSolutionProfiles(solution) {
    ...
    if (!payload) {
      ...
      const isGzip = buffer.byteLength >= 2 && bytes[0] === 0x1f && bytes[1] === 0x8b;
      if (isGzip && typeof DecompressionStream !== 'undefined') {
        const decompressedStream = new Response(new Blob([buffer]).stream().pipeThrough(new DecompressionStream('gzip')));
        payload = parseJsonText(await decompressedStream.text());
      }
    }
